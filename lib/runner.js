...
const getFiles = async (dir, searchOptions) => {
	...
	const { partition, ignoreExtensions } = searchOptions;

	const fullGlobPattern = path.join(dir, globPattern);
	const globOptions = ignoreExtensions
		? { exclude: (filePath) => ignoreExtensions.includes(path.extname(filePath)) }
		: {};

	let files;
	try {
		files = globSync(fullGlobPattern, globOptions);
	} catch {
		files = [];
	}
...
module.exports = {
	...
	_copyFiles: async function (fileList, options) {
		...
		const copiedFiles = await fileList.reduce(async (memo, file) => {
			...
			if (file) {
				...
				try {
					await cp(file, newpath, { recursive: true, force: true });
					copied.push(newpath);
				} catch (err) {
					log.error(err);
				}
	 ...
	 */
	_askYesNo: async function (question, responseOverride = false) {
		let response;
		if (responseOverride) {
	...
	clean: async function (options) {
		const pathExists = existsSync(options.paths.dest);
		if (!pathExists) {
			return;
		}
		...
		try {
			await rm(options.paths.dest, { recursive: true, force: true });
		} catch {
			return 1;
		}
	},
	...
	clone_assets: async function (options, files = null) {
		if (!files) {
			...
			files = await getFiles(options.paths.fullPathToSource, {
				...
				ignoreExtensions: ['.htm', '.html', '.css', '.scss'],
			});
	...
	rewrite_css: async function (options, files = null) {
		...
		copiedFiles.forEach((file) => {
			cssRewrite(file, options.paths.fullPathToDest, options.paths.baseurl);
		});
		return 0;
	},
	...
	serve: function (options) {
		...
		server.on('listening', () => {
			log(
				`Serving '${options.paths.baseurl}' at http://localhost:${options.serve.port}/${options.paths.baseurl}`
			);
		});
	 ...
	 * occurs, then a new build is triggered.
	...
	watch: function (options) {
		...
		chokidar.watch(options.paths.src, watchOptions).on('change', (event, filePath) => {
			log(`${filePath} has been modified (${event}). Rebuilding...`);
			rebuild();
		});
